                    ...
                    <tbody>
                        ...
                        <tr onclick="window.location='/dashboard/order-management/{{ $order['id'] }}'" >
                            ...
                            @elseif($order['status'] == 'Dibatalkan')
                                <td class="align-middle text-center"><button disabled class="btn btn-outline-danger">Dibatalkan Admin</button></td>
                            @elseif($order['status'] == 'Dibatalkan User')
                                <td class="align-middle text-center"><button disabled class="btn btn-outline-danger">Dibatalkan User</button></td>
                            @else
                                <td class="align-middle text-center"><button disabled class="btn btn-outline-success">Disetujui</button></td>
                            @endif
                            <td class="align-middle text-center">{{ $order['created_at'] }}</td>
                            <td class="align-middle text-center">
                                @if($order['status'] == 'Berhasil')
                                    <a href="/dashboard/order-management/taken/{{ $order['id'] }}"><button class="btn btn-success">Diambil Pelanggan</button></a>
                                @elseif($order['status'] == 'Checking Admin')
                                    <a href="/dashboard/order-management/approve/{{ $order['id'] }}"><button class="btn btn-primary">Setujui</button></a>
                                    <a href="/dashboard/order-management/reject/{{ $order['id'] }}"><button class="btn btn-danger">Batalkan</button></a>
                                @else($order['status'] == 'Dibatalkan' || $order['status'] == 'Dibatalkan Admin')
                                    <a><button disabled class="btn btn-danger">Dibatalkan</button></a>
                                @endif
                            </td>
                        </tr>
                        @endforeach
