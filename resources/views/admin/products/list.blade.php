...
@section('content')
    ...
    <div class="container">
        <div class="d-flex justify-content-between align-items-center">
            ...
            <div>
            <a href="/dashboard/product-management/create"><button class="btn btn-success">Tambah Obat</button></a>
                ...
                <table class="table table-bordered mt-3">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center align-middle">@currency($product['price'])</td>
                            <td class="text-center align-middle">{{ $product['stock'] }}</td>
                            <td class="text-center align-middle">{{ substr($product['expired_date'], 0, 10) }}</td>
                            <td style="white-space: nowrap; text-overflow:ellipsis; overflow: hidden; max-width:20px;" class="text-center align-middle">{{ $product['description'] }}</td>
                            @if($status[$key] == 'Expired')
                                <td id="expired_date-{{$product['id']}}" class="text-center align-middle"><button class="btn btn-outline-danger" disabled>{{ $status[$key] }}</button></td>
                            @else
                                <td id="expired_date-{{$product['id']}}" class="text-center align-middle"><button class="btn btn-outline-success" disabled>{{ $status[$key] }}</button></td>
                            @endif
                            <td class="text-center">
                                <a href="/dashboard/product-management/delete/{{ $product['id'] }}"><button class="btn btn-danger">Delete</button></a>
                                <a href="/dashboard/product-management/{{ $product['id'] }}"><button class="btn btn-primary">Edit</button></a>
                            </td>
                        </tr>
                        @endforeach
                        @if(count($products) < 1) 
